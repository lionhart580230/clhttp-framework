...
func init() {
	...
	paramCheckers[PTYPE_DATE] = func(_param string) string {
		match, err := regexp.Match(`^[0-9]{4}\-[01][1-9]\-[01][1-9]$`, []byte(_param))
		if err != nil || !match {
			return PARAM_CHECK_FAIED
		}
		return _param
	...
	paramCheckers[PTYPE_TIME] = func(_param string) string {
		match, err := regexp.Match(`^[0-2][1-9]\:[0-5][0-9]\:[0-5][0-9]$`, []byte(_param))
		if err != nil || !match {
			return PARAM_CHECK_FAIED
		}
		return _param
	...
	paramCheckers[PTYPE_DATETIME] = func(_param string) string {
		match, err := regexp.Match(`^[0-9]{4}\-[01][1-9]\-[01][1-9]\s[0-2][1-9]\:[0-5][0-9]\:[0-5][0-9]$`, []byte(_param))
		if err != nil || !match {
			return PARAM_CHECK_FAIED
		}
		return _param
