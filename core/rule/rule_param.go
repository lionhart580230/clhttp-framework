...
// 新的正则表达式参数
func NewRegExp(_name string, _static bool, _default string, _regExp string) ParamInfo {
	return ParamInfo{
		Name:      _name,
		ParamType: PTYPE_REGEX,
		Static:    _static,
		Default:   _default,
		Extra: []string{
			_regExp,
...
// 在指定列表中的字符串
// 只有在这个列表中出现的字符串才允许通过
func NewStrIn(_name string, _static bool, _default string, _strList []string, _caseMath bool) ParamInfo {
	return ParamInfo{
		Name:      _name,
		ParamType: PTYPE_STR_IN,
		Static:    _static,
		Default:   _default,
		Extra: []string{
			strings.Join(_strList, "&$&"),
			fmt.Sprintf("%v", _caseMath),
		},
	}
}
