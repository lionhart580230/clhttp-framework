...
// 设置忽略文件后缀名列表
var mIgnoreFileSuffix = []string{}

var mEnableCORS = true
...
// 设置跨域
func SetCORS(_enable bool) {
	mEnableCORS = _enable
}

func init() {
	mIgnoreFileSuffix = []string{
...
func StartServer(_listenPort uint32) {
	...
	tempDirPath, _ := os.MkdirTemp("__clhttp_tempfile__", "")
...
func rootHandler(rw http.ResponseWriter, rq *http.Request) {
	...
	// 过滤请求
	for _, filterExt := range mIgnoreFileSuffix {
