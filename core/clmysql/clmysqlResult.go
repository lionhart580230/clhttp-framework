...
func (res *TdbResult) GetDate(key string, def string) string {
	...
	if val, find := (*res)[key]; find {
		if clCommon.Uint32(val) == 0 {
			return def
		}
		return cltime.GetDateByFormat(clCommon.Uint32(val), "2006-01-02")
	} else {
		return def
	}
...
func (res *TdbResult) GetTime(key string, def string) string {
	...
	if val, find := (*res)[key]; find {
		...
		return cltime.GetDateByFormat(clCommon.Uint32(val), "15:04:05")
	} else {
		return def
	}
...
func (res *TdbResult) GetDateTime(key string, def string) string {
	...
	if val, find := (*res)[key]; find {
		...
		return cltime.GetDateByFormat(clCommon.Uint32(val), "2006-01-02 15:04:05")
	} else {
		return def
	}
